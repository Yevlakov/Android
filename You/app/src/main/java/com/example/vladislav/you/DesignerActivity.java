...
public class DesignerActivity extends AppCompatActivity {
    ...
    public void MyonCheckboxClicked(View view)
    {
...

        {
            ...
            {
                mImageView.setImageDrawable(getResources().getDrawable(R.drawable.des_no));
                Toast.makeText(this, "Вы всего лишь обычный дизайнер...", Toast.LENGTH_SHORT).show();
            }
            else
            {
                mImageView.setImageDrawable(getResources().getDrawable(R.drawable.des_yes));
                Toast.makeText(this, "Поздравляю,вы - ТЫЖдизайнер!!!", Toast.LENGTH_SHORT).show();
            }
        }
